...
class ClaimExtractionChain:
    ...
    def __init__(self, model: str = "gpt-4o-mini"):
        ...
        if not REFCHECKER_AVAILABLE:
            raise ImportError(
                "RefChecker not installed. Install with: pip install refchecker"
            )
    ...
    async def extract(
        ...
        for i, triplet in enumerate(triplets):
            ...
            claim = Claim(
                ...
                context=document[max(0, document.find(claim_text) - 100):
                                min(len(document), document.find(claim_text) + len(claim_text) + 100)]
                if claim_text in document else None
            )
            claims.append(claim)
            ...
            for i, extracted in enumerate(result.claims):
                ...
                if extracted.text.strip():
                    claims.append(Claim(
                        ...
                        source_document=source_path,
                        claim_type=extracted.claim_type if extracted.claim_type in ["explicit", "implicit", "inferred"] else "explicit"
                    ))
